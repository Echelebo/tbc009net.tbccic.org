                ...
                <div class="col-md-6 col-12 col-xl-7">
                    <div class="auth-section d-flex align-items-center">
                        <div class="auth-module">
                            <form action="{{ url('register/store') }}" class="form-horizontal" id="register-form"
                                method="POST">
                                @csrf
                                <input type="hidden" name="has_captcha"
                                    value="{{ isset($enabledCaptcha) && ($enabledCaptcha == 'registration' || $enabledCaptcha == 'login_and_registration') ? 'registration' : 'Disabled' }}">
                                <input type="hidden" name="defaultCountry" id="defaultCountry" class="form-control"
                                    value="{{ $formInfo['defaultCountry'] }}">
                                <input type="hidden" name="carrierCode" id="carrierCode" class="form-control"
                                    value="{{ $formInfo['carrierCode'] }}">
                                <input type="hidden" name="formattedPhone" id="formattedPhone" class="form-control"
                                    value="{{ $formInfo['formattedPhone'] }}">
                                <input type="hidden" name="first_name" id="first_name" class="form-control"
                                ...
                                <input type="hidden" name="password_confirmation" id="password_confirmation"
                                    class="form-control" value="{{ $formInfo['password_confirmation'] }}">
                                ...
                                <div class="auth-module-header">
                                    ...
                                    <p class="mb-0 auth-text text-start mt-12 leading-24">
                                        {{ __('Select user account for personal use and merchant account for business payments.') }}
                                    </p>
                                </div>

                                <div class="row">
                                    <div class="col-md-12">
                                        <p class="mb-0 type-text mt-3p">{{ __('Select Account Type') }}<span
                                                class="star">*</span></p>
                                        <div class="d-flex gap-16 mt-2 radio-hide">
                                            @if (!empty($checkUserRole))
                                                ...
                                                <label for="user"
                                                    class="usertype-module usertype-photo d-inline-flex flex-column justify-content-center align-items-center user-selected">
                                                    <img src="{{ asset('public/frontend/templates/images/login/woman-user.png') }}"
                                                        alt="user">
                                                    <p class="usertype mt-22">{{ __('User') }}</p>
                                                </label>
                                            @endif
                                            @if (!empty($checkMerchantRole))
                                                <input type="radio" name="type" id="merchant" value="merchant"
                                                    required=""
                                                    oninvalid="this.setCustomValidity('{{ __('This field is required.') }}')">
                                                <label for="merchant"
                                                    class="usertype-module usertype-photo d-inline-flex flex-column justify-content-center align-items-center">
                                                    <img src="{{ asset('public/frontend/templates/images/login/men-merchant.png') }}"
                                                        alt="merchant">
                                        ...
                                        <div class="policy d-flex justify-content-center mt-28">
                                            <p class="mb-0">
                                                {{ __('By signing up, you agree to our Terms, Data Policy and Cookie Policy.') }}
                                            </p>
                                        </div>
                                        <div class="d-grid sm-top mt-28">
                                            <button class="btn btn-lg btn-primary" type="submit"
                                                ...
                                                <span class="px-1"
                                                    id="registrationSubmitBtnTxt">{{ __('Create Account') }}</span>
